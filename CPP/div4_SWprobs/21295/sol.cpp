...
ll Sum(ll N,int K) {
    ...
    if(K == 1) {
        ...
        ll div = 5;
        return Sum(N,2) * (3 * N * N + 3 * N - 1) / div;
    }
}
...
int main() {
    ...
    for(ll i = 1;i <= 1000000;i++) {
        value[i][0] = i;
        value[i][1] = i*i;
        value[i][2] = i * value[i][1];
        value[i][3] = value[i][1] * value[i][1];
    }
    ...
    for(int t = 1;t <= T;t++) {
        ...
        cout << int128ToString(Sum(1000000,4)) << endl;
        int N;cin >> N;
        int K; cin >> K;
