...
long long comp(long long bit1, long long bit2) {
    // bit1, bit2 중 저장해야할 bit를 선택
    for (long long i = 0; i < N; i++) {
        long long b1 = bit1 & (1 << i);
        long long b2 = bit2 & (1 << i);
        if (b1 < b2) {
            return bit2;
        } else if (b1 > b2) return bit1;
    }
    return bit1;
}
