...
int add(int mId, int mGrade, char mGender[7], int mScore) {
	...
	int highestScore = (*it).score;
	while(it-1 >= arr[mGrade-1][gender].begin() && (*(it-1)).score == highestScore) {
		it--;
	}
...
int remove(int mId) {
	...
	int lowestScore = arr[grade][gender][0].score;
	it = arr[grade][gender].begin();
	while(it+1 != arr[grade][gender].end() && (*(it+1)).score == lowestScore) {
		it++;
	}
	return (*it).id;
}
...
int query(int mGradeCnt, int mGrade[], int mGenderCnt, char mGender[][7], int mScore) { //남자 0 여자 1
	...
	for(int gc = 0;gc < mGradeCnt;gc++) {
		...
		for(int gender = 0;gender < 2;gender++) {
			...
			cout << "Checking " << grade << " " << gender << endl;
			...
			if(it != arr[grade][gender].end()) {
				...
				cout << "candidate added " << (*it).id << endl;
			}
		}
	}
	...
	int lowestScore = (*it).score;
	while(it+1 != candidates.end() && (*(it+1)).score  == lowestScore) {
		it++;
	}
	return (*it).id;
}