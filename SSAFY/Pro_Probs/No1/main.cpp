...
static int run()
{
	...
	for (int c = 0; c < N; ++c)
	{
		...
		case CMD_ALLY:
			...
			if (result != check) {
				isOK = 0;
				printf("Ally %s %s failed. expected : %d, returned : %d\n",mMonarchA,mMonarchB,check,result);
			}
			break;
		...
		case CMD_ATTACK:
			...
			if (result != check) {
				isOK = 0;
				printf("ATTACK %s %s %s failed. expected : %d, returned : %d\n",mMonarchA,mMonarchB,mGeneral,check,result);
			}
			break;
		...
		case CMD_RECRUIT:
			...
			if (result != check) {
				isOK = 0;
				printf("RECRUIT %s %d %d failed. expected : %d, returned : %d\n",mMonarchA,num,mOption,check,result);
			}
